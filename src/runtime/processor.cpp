...
#include <rex/runtime/thread.h>
#include <rex/runtime/thread_state.h>
#include <rex/runtime/guest/context.h>

REXCVAR_DEFINE_BOOL(disable_global_lock, false,
    "Disable global threading lock",
    "Runtime");
...
uint64_t Processor::Execute(ThreadState* thread_state, uint32_t address,
  ...
  if (arg_count > 0) ctx->r3.u64 = args[0];
  if (arg_count > 1) ctx->r4.u64 = args[1];
  if (arg_count > 2) ctx->r5.u64 = args[2];
  if (arg_count > 3) ctx->r6.u64 = args[3];
  if (arg_count > 4) ctx->r7.u64 = args[4];
  if (arg_count > 5) ctx->r8.u64 = args[5];
  if (arg_count > 6) ctx->r9.u64 = args[6];
  if (arg_count > 7) ctx->r10.u64 = args[7];
...
uint64_t Processor::ExecuteInterrupt(ThreadState* thread_state,
  ...
  auto pcr_address =
      memory_->TranslateVirtual(static_cast<uint32_t>(ctx->r13.u64));
...
void Processor::OnThreadCreated(uint32_t thread_handle,
  ...
  REXLOG_DEBUG("Thread created: handle={:08X}, id={}", thread_handle,
...
bool Processor::InitializeFunctionTable(uint32_t code_base, uint32_t code_size,
