...
#ifdef __APPLE__
// Apple libc++ doesn't have std::chrono::clock_cast, provide inline helpers
namespace {
using rex::chrono::WinSystemClock;
using rex::chrono::XSystemClock;
inline WinSystemClock::time_point xsys_to_win(XSystemClock::time_point t) {
  std::atomic_thread_fence(std::memory_order_acq_rel);
  auto w_now = WinSystemClock::now();
  auto x_now = XSystemClock::now();
  std::atomic_thread_fence(std::memory_order_acq_rel);
  auto delta = std::chrono::floor<WinSystemClock::duration>(t - x_now);
  return w_now + delta;
}
}  // namespace
#define REX_CLOCK_CAST_WIN(tp) xsys_to_win(tp)
#else
#define REX_CLOCK_CAST_WIN(tp) std::chrono::clock_cast<WinSystemClock>(tp)
#endif
...
X_STATUS XTimer::SetTimer(int64_t due_time, uint32_t period_ms, uint32_t routine,
  ...
  if (due_time < 0) {
    ...
    due_tp = REX_CLOCK_CAST_WIN(XSystemClock::now() + after);
  } else {
    due_tp = REX_CLOCK_CAST_WIN(XSystemClock::from_file_time(due_time));
  }
