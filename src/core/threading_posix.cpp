...
#ifdef __APPLE__
// macOS doesn't have real-time signals; use SIGUSR1/SIGUSR2 as base
#ifndef SIGRTMIN
#define SIGRTMIN SIGUSR1
#define SIGRTMAX (SIGUSR1 + 2)
#endif
#endif
...
class PosixCondition<Thread> : public PosixConditionBase {
 ...
 public:
  ...
  void set_name(const std::string& name) {
    ...
    if (state_ != State::kUninitialized && state_ != State::kFinished) {
#ifdef __APPLE__
      // macOS pthread_setname_np only sets current thread name; skip for other threads
      (void)name;  // will be set when the thread itself calls set_current_thread_name
#else
  ...
  uint32_t system_id() const { return static_cast<uint32_t>(reinterpret_cast<uintptr_t>(thread_)); }

  uint64_t affinity_mask() {
    WaitStarted();
#ifdef __APPLE__
    (void)0;  // CPU affinity not supported on macOS
    return 0;
#else
    cpu_set_t cpu_set;
#if REX_PLATFORM_ANDROID
  ...
  void set_affinity_mask(uint64_t mask) {
    WaitStarted();
#ifdef __APPLE__
    (void)mask;  // CPU affinity not supported on macOS
    return;
#else
    cpu_set_t cpu_set;
    CPU_ZERO(&cpu_set);
  ...
  void QueueUserCallback(std::function<void()> callback) {
    ...
    // macOS has no pthread_sigqueue; send signal directly (no sigval payload)
...
class PosixTimer : public PosixConditionHandle<Timer> {
 ...
 public:
  ...
  bool SetOnceAt(WClock_::time_point due_time,
                 std::function<void()> opt_callback = nullptr) override {
    // clock_cast not available in Apple libc++; manual conversion via now()
    auto wdelta = due_time - WClock_::now();
    auto gdue = GClock_::now() + std::chrono::duration_cast<GClock_::duration>(wdelta);
    return SetOnceAt(gdue, std::move(opt_callback));
  };
  ...
  bool SetRepeatingAt(WClock_::time_point due_time, std::chrono::milliseconds period,
    ...
    auto wdelta = due_time - WClock_::now();
    auto gdue = GClock_::now() + std::chrono::duration_cast<GClock_::duration>(wdelta);
    return SetRepeatingAt(gdue, period, std::move(opt_callback));
  }
