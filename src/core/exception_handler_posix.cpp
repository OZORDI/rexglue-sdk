...
static void ExceptionHandlerCallback(int signal_number, siginfo_t* signal_info,
...
#elif REX_ARCH_ARM64
#ifdef __APPLE__
  // macOS ARM64 mcontext structure
  for (int i = 0; i < 29; i++) {
  ...
  std::memcpy(thread_context.v, &mcontext->__ns.__v[0], sizeof(thread_context.v));
  // No ESR context on macOS signal handlers
  uint64_t mcontext_esr_val = 0;
  bool has_esr = false;
#else
  ...
  uint64_t mcontext_esr_val = mcontext_esr ? mcontext_esr->esr : 0;
  bool has_esr = (mcontext_esr != nullptr);
#endif
#endif  // REX_ARCH
  ...
  switch (signal_number) {
    ...
    case SIGSEGV: {
      ...
      // memory location.
      if (has_esr && ((mcontext_esr_val >> 26) & 0b111110) == 0b100100) {
        access_violation_operation = (mcontext_esr_val & (UINT64_C(1) << 6))
                                         ? Exception::AccessViolationOperation::kWrite
                                         : Exception::AccessViolationOperation::kRead;
      } else {
