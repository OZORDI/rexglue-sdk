...
#if defined(REX_HEADLESS)
#include <atomic>
static uint32_t s_headless_gpu_callback = 0;
static uint32_t s_headless_gpu_callback_data = 0;
static rex::kernel::object_ref<rex::kernel::XHostThread> s_headless_gpu_timer_thread;
static std::atomic<bool> s_headless_gpu_timer_running{false};
#endif
...
#if defined(REX_HEADLESS)
  s_headless_gpu_callback = callback;
  s_headless_gpu_callback_data = user_data;
  REXKRNL_INFO("[HEADLESS] GPU interrupt callback registered: {:08X}", (uint32_t)callback);

  if (!s_headless_gpu_timer_running.exchange(true)) {
    s_headless_gpu_timer_thread = kernel::object_ref<kernel::XHostThread>(
        new kernel::XHostThread(kernel_state(), 64 * 1024, 0, []() {
          while (s_headless_gpu_timer_running.load(std::memory_order_relaxed)) {
            rex::thread::Sleep(std::chrono::milliseconds(16));
            if (!s_headless_gpu_timer_running.load(std::memory_order_relaxed)) break;
            uint32_t cur_cb = s_headless_gpu_callback;
            if (cur_cb) {
              auto* thread = kernel::XThread::GetCurrentThread();
              if (thread) {
                thread->SetActiveCpu(2);
                uint64_t args[] = {0, s_headless_gpu_callback_data};
                kernel_state()->emulator()->processor()->ExecuteInterrupt(
                    thread->thread_state(), cur_cb, args, 2);
              }
            }
          }
          return 0;
        }));
    s_headless_gpu_timer_thread->set_name("Headless GPU Timer");
    s_headless_gpu_timer_thread->Create();
    s_headless_gpu_timer_thread->Resume();
  }
#else
