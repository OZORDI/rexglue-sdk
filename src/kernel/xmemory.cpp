...
bool Memory::Initialize() {
  ...
  heaps_.v00000000.AllocFixed(
      ...
      // Headless: rendering code dereferences NULL D3D pointers (never
      // created because graphics system is absent). Allow R/W to avoid
      // thousands of signal handler faults per second.
...
#if defined(REX_HEADLESS)
  // In headless mode, no GraphicsSystem or AudioSystem registers MMIO ranges,
  // so accesses to GPU/audio register pages cause faults handled by the MMIO
  // signal handler. Committing these pages as R/W memory filled with 0xFF
  // eliminates signal handler overhead AND satisfies hardware-ready polling
  // loops (which check for non-zero / bit-set values). Zeroed pages would
  // cause infinite loops since the per-PC fault budget only works on faults.
  // These addresses are in the gap between v40000000 and v80000000 heaps
  // (0x7F000000-0x7FFFFFFF), so we commit directly on host addresses.
  {
    // GPU registers: 0x7FC80000, 64KB
    void* gpu_result = rex::memory::AllocFixed(
        virtual_membase_ + 0x7FC80000, 0x10000,
        rex::memory::AllocationType::kCommit,
        rex::memory::PageAccess::kReadWrite);
    if (gpu_result) {
      std::memset(gpu_result, 0xFF, 0x10000);
      REXKRNL_DEBUG("Committed GPU MMIO pages at 0x7FC80000 (filled 0xFF) for headless mode");
    } else {
      REXKRNL_WARN("Failed to commit GPU MMIO pages at 0x7FC80000");
    }
    // Audio registers: 0x7FEA0000, 64KB
    void* audio_result = rex::memory::AllocFixed(
        virtual_membase_ + 0x7FEA0000, 0x10000,
        rex::memory::AllocationType::kCommit,
        rex::memory::PageAccess::kReadWrite);
    if (audio_result) {
      std::memset(audio_result, 0xFF, 0x10000);
      REXKRNL_DEBUG("Committed audio MMIO pages at 0x7FEA0000 (filled 0xFF) for headless mode");
    } else {
      REXKRNL_WARN("Failed to commit audio MMIO pages at 0x7FEA0000");
    }
  }
#endif  // REX_HEADLESS
