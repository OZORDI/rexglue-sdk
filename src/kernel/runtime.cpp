...
#undef LOAD_KERNEL_MODULE
  ...
  if (gpu_backend == "any") {
    ...
    if (!graphics_system_) {
      ...
      REXKRNL_INFO("Auto-selected Vulkan GPU backend (fallback)");
    }
  }
#if REX_HAS_D3D12
  else if (gpu_backend == "d3d12") {
    if (!graphics::d3d12::D3D12GraphicsSystem::IsAvailable()) {
      REXKRNL_ERROR("D3D12 GPU backend requested but not available");
      return X_STATUS_UNSUCCESSFUL;
    }
    graphics_system_ = std::make_unique<graphics::d3d12::D3D12GraphicsSystem>();
    ...
    if (!graphics::metal::MetalGraphicsSystem::IsAvailable()) {
      REXKRNL_ERROR("Metal GPU backend requested but not available");
      return X_STATUS_UNSUCCESSFUL;
    }
    graphics_system_ = std::make_unique<graphics::metal::MetalGraphicsSystem>();
